with cbe as (SELECT sum(lot_qty) td_qty, account, symbol
             FROM (SELECT x.*,
                          row_number()
                          over (partition BY account, symbol, unique_opening_record_id, trade_no, related_trd_no ORDER BY maxit_trsn_id desc nulls last) rn
                   FROM STG_POSTTRADE.REFINITIV_CBE_DB x
                   WHERE file_name = 'DRW_VM_PLUTO_MASTER_dat_20240513'
                     AND partner in ('CASH'))
             WHERE rn = 1
               AND lot_status = 'O'
               AND update_type <> 'D'
             GROUP BY account, symbol),
     filters as (SELECT y.account                                                iaccount,
                        y.symbol                                                 isymbol,
                        x.ACCT_NO                                                accountno,
                        cast(nvl(round(y.td_qty, 8), 0) as number(28, 8)) -
                        cast(nvl(round(x.tclose, 8), 0) as number(28, 8))        break_qty,
                        case when break_qty <> 0 then 'UNMATCH' else 'MATCH' end break_type
                 FROM stg_icdb.pos_bal x
                          JOIN cbe y
                               ON trim(x.corr) || trim(x.office) || trim(x.acct_no) = y.account AND x.symbol = y.symbol
                 WHERE x.system_dt = '2024-05-13'
                   AND corr in ('CASH'))
SELECT x.account_no                                             as                                                                account,
       x.partner                                                as                                                                partner,
       x.symbol                                                 as                                                                symbol,
       (case when x.account_type_ind = 1 then 'C' else 'M' end) as                                                                acct_type,
...
FROM STG_POSTTRADE.REFINITIV_CBE_DB x
WHERE file_name = 'DRW_VM_PLUTO_MASTER_dat_20240513'
  and (account, symbol) IN (select iaccount, isymbol from filters where break_type = 'MATCH')
UNION ALL
SELECT account,
       PARTNER,
       symbol,
       ...
       src,
       rn
FROM (SELECT dpos.ACCOUNTNO                                         as                                                 account,
             p.WLPID                                                as                                                 PARTNER,
             ins.symbol                                             as                                                 symbol,
             (case when dpos.ACCOUNTTYPE = 1 then 'C' else 'M' end) as                                                 acct_type,
             ...
             dpos.eventtype,
             row_number()
             over (partition BY dpos.accountid, dpos.INSTRUMENTID, dpos.ACCOUNTTYPE ORDER BY EVENT_TS desc nulls last) rn
      ...
      where (dpos.accountno, ins.symbol) IN (select accountno, isymbol from filters where break_type = 'UNMATCH')
        and EVENT_TS < '2024-05-13 23:59:59.000')
where rn = 1
  and eventtype <> 'REMOVE'